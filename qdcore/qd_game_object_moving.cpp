...
bool qdGameObjectMoving::move(const Vect3f &target, bool lock_target) {
	debugC(3, kDebugMovement, "Move to %f %f %f %d", target.x, target.y, target.z, lock_target);

	set_last_move_order(target);
	if (enough_far_target(target) == false)
		return true;
...
bool qdGameObjectMoving::find_path(const Vect3f target, bool lock_target) {
	debugC(3, kDebugMovement, "Find path to %f %f %f", target.x, target.y, target.z);
	Vect3f trg = target;
	...
	debugC(3, kDebugMovement, "qdGameObjectMoving::find_path %d", is_walkable(trg));
...
bool qdGameObjectMoving::is_walkable(const Vect2s &pos) const {
	debugC(3, kDebugMovement, "qdGameObjectMoving::is_walkable %d %d", pos.x, pos.y);
...
bool qdGameObjectMoving::set_walk_animation() {
	debugC(5, kDebugMovement, "Set walk animation");
	float tm = 0.0f;
...
bool qdGameObjectMoving::set_movement_impulse(float dir_angle) {
	debugC(5, kDebugMovement, "qdGameObjectMoving::set_movement_impulse");
...
bool qdGameObjectMoving::movement_impulse() {
	debugC(3, kDebugMovement, "Movement impulse");
